...
app.post('/api/fees/mark-paid', async (req, res) => {
  try {
    ...
    let record = await Attendance.findOne({ date: dateKey });
    if (!record) {
      record = new Attendance({ date: dateKey, sessions: {} });
    }
    if (!record.paidMonthsLog) record.paidMonthsLog = {};

    const key = String(studentId);
    const existing = record.paidMonthsLog[key] || [];
    keysToAdd.forEach((mk) => {
      if (!existing.includes(mk)) existing.push(mk);
    });
    record.paidMonthsLog[key] = existing;
    record.markModified('paidMonthsLog');

    await record.save();
    res.json({ message: 'Payment recorded', studentId, monthKeys: keysToAdd });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/fees/unmark-paid', async (req, res) => {
  try {
    ...
    if (!studentId || !monthKey) {
      return res.status(400).json({ message: 'studentId and monthKey are required' });
    }

    const records = await Attendance.find({ [`paidMonthsLog.${studentId}`]: { $exists: true } });
    const key = String(studentId);

    for (const record of records) {
      if (record.paidMonthsLog && record.paidMonthsLog[key]) {
        record.paidMonthsLog[key] = record.paidMonthsLog[key].filter((m) => m !== monthKey);
        record.markModified('paidMonthsLog');
        await record.save();
      }
    }

    res.json({ message: 'Payment mark removed', studentId, monthKey });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/fees/paid-months', async (req, res) => {
  try {
    ...
    records.forEach((r) => {
      if (r.paidMonthsLog) {
        Object.entries(r.paidMonthsLog).forEach(([studentId, months]) => {
          if (!result[studentId]) result[studentId] = new Set();
          months.forEach((m) => result[studentId].add(m));
        });
      }
    });

    const formatted = {};
    Object.entries(result).forEach(([studentId, monthSet]) => {
      formatted[studentId] = Array.from(monthSet).sort();
    });
