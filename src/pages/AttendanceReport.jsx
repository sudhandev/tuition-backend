...
import { Calendar, UserRound, ArrowLeft } from "lucide-react";
...
export default function AttendanceReport() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`${API_URL}/api/all-attendance`).then((res) =>
        res.json(),
      ),
    ])
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-400 mt-1">
              Audit logs and past attendance sheets across all recorded dates
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {recordedDates.map((date) => {
              const dayRecord = allAttendanceData[date]?.attendance || {};
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 pb-4 mb-4 border-b border-slate-100 text-purple-700 font-bold text-sm">
                    <Calendar size={18} />
                    <span>Attendance Sheet for: {date}</span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {recordedStudentIds.map((studentId) => {
                      const record = dayRecord[studentId];
                      // Support both old format (string status) and new format (object with name & status)
                      const status =
                        typeof record === "object" ? record.status : record;

                      // Fallback chain: Saved record name -> Active student list -> Fallback ID
