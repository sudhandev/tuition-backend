...
import { AlertCircle, Calendar, DollarSign, UserRound, ArrowLeft, Plus, Edit2, Trash2, Check, Sun, Moon } from "lucide-react";
...
export default function FineTracker() {
  ...
  const totalCollectedAmount = finesList.reduce((sum, item) => sum + (Number(item.amount) || 0), 0);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            {/* Quick Stat Pill */}
            <div className="flex items-center gap-3 bg-white border border-slate-200/80 px-4 py-2.5 rounded-2xl shadow-xs">
              <div className="w-9 h-9 rounded-xl bg-red-100 flex items-center justify-center text-red-600 font-bold">
                <DollarSign size={18} />
              </div>
              <div>
                <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Total Fines</p>
                <p className="text-base font-extrabold text-slate-800">₹{totalCollectedAmount}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3.5">
              {finesList.map((item) => (
                <div 
                  key={item.id} 
                  className="group flex flex-col sm:flex-row sm:items-center justify-between p-4.5 rounded-2xl bg-slate-50/60 border border-slate-200/60 hover:border-purple-200 hover:bg-purple-50/20 transition-all gap-4"
                >
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-5 border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-200/60">
                    <span className="text-lg font-black text-red-600 bg-red-50 px-3 py-1 rounded-xl border border-red-100">
                      ₹{item.amount}
                    </span>
